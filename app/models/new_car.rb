class NewCar < ApplicationRecord
  ...
  def discount_count(e = self.equipments.where('equipment.hide = FALSE').order(price: :asc).first)
    #self.discount.to_i + self.discount_tradein.to_i + self.discount_credit.to_i + 50000 + 30000 +(e.price * 0.1).to_i
    self.discount.to_i + self.discount_tradein.to_i + self.discount_credit.to_i + 40000
  end

  def count_price(e = self.equipments.where('`equipment`.`hide` = ?', 0).minimum(:price),reg_discount = 0)
    price = e
    #discount = self.discount.to_i + self.discount_tradein.to_i + self.discount_credit.to_i + 50000 + 30000 + (e * 0.1).to_i
    discount = self.discount.to_i + self.discount_tradein.to_i + self.discount_credit.to_i + 40000
...
# WHERE `new_cars`.`hide` = 0 AND `new_cars`.`id` != 291 AND equipment.price != 1 AND equipment.price < (900000 + `new_cars`.`discount` + `new_cars`.`discount_credit` + `new_cars`.`discount_tradein` )
...
# LIMIT 25
  ...
  scope :price_range, -> (range_lower,range_upper) do
    select('(min(equipment.price) - `new_cars`.`discount` - `new_cars`.`discount_credit` - `new_cars`.`discount_tradein` - 30000) as min_price,`new_cars`.*')
